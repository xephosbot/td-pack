...
if [ "$OS" = "linux" ]; then
  if [ "$ARCH" = "x86_64" ]; then
    ...
    cmake --build --preset build-linux-x86_64-install -S td
  elif [ "$ARCH" = "arm64" ] || [ "$ARCH" = "aarch64" ]; then
    # prepare generated files using native build
    conan install . -pr:b=profiles/linux_x86_64 -pr:h=profiles/linux_x86_64 --build=missing
    source build/conan/Release/generators/conanbuild.sh
    cmake --preset linux-x86_64
    cmake --build --preset build-linux-aarch64-prepare -S td
    ...
    cmake --build --preset build-linux-aarch64-install -S td
  else
  ...
  if [ "$ARCH" = "arm64" ]; then 
    ...
    cmake --build --preset build-macos-arm64-install -S td
    ...
    cmake --build --preset build-macos-x86_64-install -S td
  else
